...
version: 1.1.0
mode: agent
description: Run the bounded verify loop - implement, check against the governing checklist, fix, repeat, capped at two iterations.
---

Run the verify loop defined in `.github/loops/verify.md` for the task described below.

Before the first iteration, state:

- the task in one sentence
- which skill checklist governs it, the targeted tests you will run while
  iterating, and the applicable build, lint and test commands you will run once
  on the final tree
- the cap you are working under: 2 iterations, 60k tokens
...
Stop and escalate if two iterations are used, the budget is exhausted, the
