---
mode: agent
description: Consolidate the pending material from closed sessions into candidate decisions only on an explicit request.
version: 1.0.0
---

Run `node .github/hooks/scripts/dream-collect.mjs` to collect first. Then read `.harness/dream-pending.json`; if it does not exist, say
...
`.github/templates/dreams.md`. Nothing is written to `<specs>/_decisions.md`; promotion
