 ...
 * the wrapper.
 */
import { readHookInput, isHookMode, collectStrings, allow, ask, deny, EXIT_OK } from './lib/io.mjs';
...
const hit = isDryRun ? null : rules.find((r) => r.when());

process.exit(
  hit === null || hit === undefined
    ? allow('PreToolUse')
    : hit.decision === 'deny'
      ? deny('PreToolUse', hit.reason)
      : ask('PreToolUse', hit.reason)
);
