...
failed, repeat. Bounded at two iterations.

Budget: see `budgets.json`. Two iterations, 60k tokens, stop when every
applicable checklist item passes and the appropriate checks pass
on the final tree.

## Why it is bounded

Each iteration resends the accumulated history, so cost grows faster than the
number of turns. Two passes is where the return stops justifying the spend: if
a check still fails on the second attempt, the problem is the understanding, not
...
same content, command, configuration and environment, or improve what already works. A check that passed stays passed
while that identity is compatible; a file that was read stays read until it is edited.
...
3. **Judge.** Something failed: continue. Everything passed: run the applicable checks once, on this final tree. Green means stop and report.
...
applicable checks passed once on the final tree. Do not start another
...
- the suite is red for a reason outside the change, so the change cannot be
  proven
- a fix would require violating a hard constraint in the workspace `AGENTS.md` contract
